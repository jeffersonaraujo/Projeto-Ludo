...
/**
 * Classe Tabuleiro, onde é composto por várias casas e vai ser usado para os 
 * jogadores jogarem.
 *
...
public class Tabuleiro {
     ...
     * 
     * @param casa Casa
     * @throws TabuleiroException Lança um exceção caso a casa não esteja 
    ...
    public void adicionaCasa(Casa casa) throws TabuleiroException {
        ...
        if (isDuplicate(casa.getId())) {
            throw new TabuleiroException("Ja existe uma casa com ID '"
                    + casa.getId() + "'");
        }
        ...
        if (casa.getTipoCasa().equals("IR_PARA")) {
            ...
            if (!idExiste(casa.getParam())) {
                throw new TabuleiroException("Nao existe casa com id '"
                        + casa.getParam() + "'");
            }
        }
     ...
     */
    public void verificaInteiroVolta(String param) throws TabuleiroException {
        try {
            Integer.parseInt(param);
        } catch (NumberFormatException ex) {
            throw new TabuleiroException("Numero de casas para voltar invalido: "
                    + "'" + param + "'", ex);
        }
    }
     ...
     * @throws TabuleiroException lança exceção se o parametro não for um número 
     */
    public void verificaInteiroAvanca(String param) throws TabuleiroException {
        try {
            Integer.parseInt(param);
        } catch (NumberFormatException ex) {
            throw new TabuleiroException("Numero de casas para avancar invalido:"
                    + " '" + param + "'", ex);
        }
    }
     ...
     */
    public void idCasaValido(String id) throws TabuleiroException {
        if (id == null || id.equals("")) {
            throw new TabuleiroException("ID de casa invalido: '" + id + "'");
        }
    }
     ...
     * de casa abaixo.
     */
    public boolean casaInvalida(String tipoCasa) throws TabuleiroException {
        boolean retorno = false;
        if (!tipoCasa.equals("INICIO")
                ...
                && !tipoCasa.equals("FIM")) {
            retorno = true;
            throw new TabuleiroException("Tipo de casa invalido: '" + tipoCasa
                    + "'");
        }
        return retorno;
    }
